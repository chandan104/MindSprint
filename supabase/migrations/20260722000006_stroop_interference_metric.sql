-- Additive metric: stroop_interference_ms = mean reaction time on incongruent
-- trials − mean on congruent trials (correct responses only), for Colour
-- Selector Stroop rounds tagged with question_displayed.congruent. Null when a
-- session lacks congruency-tagged trials of both kinds; jsonb_strip_nulls drops
-- it there, so every existing session's metrics stay byte-identical and all
...
begin
  ...
  -- correct tap's reaction time (question -> tap), split by congruency.
  with q as (
    select seq, t_ms, (payload ->> 'congruent')::boolean as congruent
      from public.session_events
     where session_id = p_session_id and event_type = 'question_displayed'
       and payload ? 'congruent'
  ),
  paired as (
    select t.t_ms - ql.t_ms as rt, ql.congruent
      from public.session_events t
      join lateral (
        select q.congruent, q.t_ms from q where q.seq < t.seq
         order by q.seq desc limit 1
      ) ql on true
     where t.session_id = p_session_id
       and t.event_type = 'tap_registered'
       and (t.payload ->> 'is_correct')::boolean
  )
  select round(
           avg(rt) filter (where not congruent)
           - avg(rt) filter (where congruent)
         )::int
    into v_interference
    from paired;
  ...
  values
    (p_session_id,
     ...
     jsonb_strip_nulls(jsonb_build_object(
       ...
       'stroop_interference_ms', v_interference,
